...
class LearnPressImport
{
    ...
    public function learnpress_values_import($post_values, $post_id, $type, $mode)
    {
        ...
        if ($type == 'lp_course') {
            ...
            $course_setting_array['_lp_regular_price'] = isset($post_values['_lp_regular_price']) ? $post_values['_lp_regular_price'] : null;
    ...
    public function insert_question_details($type, $inserted_section_id, $question_post_id, $post_values, $temp, $mode, $condition)
    {
        ...
        if (isset($post_values['quiz_id'])) {
            ...
            $question_order = 1;
            $get_question_order = isset($quiz_id) ? $wpdb->get_var("SELECT question_order FROM {$wpdb->prefix}learnpress_quiz_questions WHERE quiz_id = $quiz_id ORDER BY quiz_question_id DESC LIMIT 1") : null;

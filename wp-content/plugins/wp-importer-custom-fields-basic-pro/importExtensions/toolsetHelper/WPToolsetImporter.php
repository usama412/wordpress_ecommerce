		...
		if($fields_Types == 'image' || $fields_Types == 'file') {
			...
			foreach($valuesArray as $value) {
				if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i',$value, $matchedlist, PREG_PATTERN_ORDER ) ) {	
					$attachid = self::$mediaInstance->image_meta_table_entry($post_val, $postId, $metaKey, $value, $hash_key, 'types', isset($get_import_type) ? $get_import_type : '',$templatekey,$gmode);
					$attachids[] =$attachid;

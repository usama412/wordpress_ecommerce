...
class ACFExtension extends ExtensionHandler{
	...
	*/
	public function processExtension($data){
		...
		if(is_plugin_active('acf-repeater/acf-repeater.php')){
			$acf_group_fields = $this->ACFFields($import_type  , 'RF');		
			$response['acf_repeater_fields'] = $acf_group_fields;
		}		
	...
	*/
	public function ACFFields($import_type ,$group) {	
		global $wpdb;
		$acf_extension = new ACFProExtension();
			...
			if(!empty($rule)) {
				if ($import_type != 'Users') {
					...
					foreach($rule['location'] as $key => $value) {
						...
						else if( $value[0]['operator'] == '==' && $value[0]['param'] == 'current_user'){
							$group_id_arr[] = $group_rules->ID;
						}
					}
				}
			...
			foreach($group_id_arr as $groupId) {
				...
				if ( ! empty( $get_acf_fields ) ) {
					...
					foreach ( $get_acf_fields as $acf_pro_fields ) {
						...
						else if ( $get_field_content['type'] == 'repeater' ) {													
				...
				if ( ! empty( $get_acf_repeater_fields ) ) {
					foreach ( $get_acf_repeater_fields as $acf_pro_repeater_fields ) {
						...
						if($get_sub_field_content['type'] == 'repeater') {
							$repeaterSubFields = $acf_extension->fetchACFProRepeaterFields($acf_pro_repeater_fields->ID);
	...
	*/
	public function extensionSupportedImportType($import_type){
		if(is_plugin_active('advanced-custom-fields/acf.php') || is_plugin_active('acf-repeater/acf-repeater.php')){
